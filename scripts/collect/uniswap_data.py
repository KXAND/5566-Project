...
# Uniswap V2 真实池子地址 (USDC/ETH)
# 0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc - USDC/ETH pair
UNISWAP_EVENTS = (
    EventConfig(
        provider="uniswap_v2",
        provider_address="0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc",
        event_name="Swap",
        event_signature="Swap(address,address,uint256,uint256,uint256,uint256,address)",
        indexed_layout=("sender", "to"),
        data_layout=("uint256", "uint256", "uint256", "uint256", "address"),
        pattern="swap_event",
        pattern_version="v1",
    ),
    EventConfig(
        ...
        provider_address="0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc",
        event_name="Flash",
        event_signature="Flash(address,address,uint256,uint256,uint256)",
        indexed_layout=("sender", "to"),
...
def decode_uniswap_log(raw_log: dict, event: EventConfig, client: EtherscanClient) -> dict:
    ...
    if event.event_name == "Swap":
        ...
        amount0_in = int(decoded_values[0])
        amount1_in = int(decoded_values[1])
        amount0_out = int(decoded_values[2])
        amount1_out = int(decoded_values[3])
        return {
            "tx_hash": tx_hash,
...
def collect_recent_uniswap_events(client: EtherscanClient, target_transactions: int, initial_window: int, offset: int) -> List[dict]:
    log_status("正在连接 Etherscan 获取最新区块高度...")
    latest_block = client.get_latest_block()
    log_status(f"最新区块高度为: {latest_block}。开始向后扫描 Uniswap 事件...")
        ...
        for event in UNISWAP_EVENTS:
            ...
            while True:
                ...
                if not logs:
                    break
                for raw_log in logs:
                    batch_logs.append(decode_uniswap_log(raw_log, event, client))
                if len(logs) < offset:
                    break
                page += 1
...
def build_output(records: List[dict]) -> dict:
    ...
    return {
        "query": {"chain": "ethereum", "source": "etherscan", "protocol": "uniswap", "target_transaction_count": DEFAULT_TARGET_TRANSACTIONS},
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch Uniswap events from Etherscan.")
...
def main() -> None:
    ...
    records = collect_recent_uniswap_events(
        client=client,
    ...
    if args.output:
        ...
        output_path = os.path.join(script_dir, f"uniswap_events_{timestamp}.json")
